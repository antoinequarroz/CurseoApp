...
export function RecetteCard({
  ...
  const imageHeight =
    variant === 'hero' ? (isSmall ? 220 : 260) : variant === 'heroCompact' ? (isSmall ? 150 : 175) : isSmall ? 180 : 200;
  const [imageEnEchec, setImageEnEchec] = useState(false);

  return (
    <Card style={variant !== 'default' ? { borderRadius: 28, borderTopLeftRadius: 28 } : undefined}>
      ...
      <View style={{ padding: variant === 'hero' ? 20 : variant === 'heroCompact' ? 14 : 16, gap: variant === 'heroCompact' ? 5 : 8 }}>
        ...
        {variant !== 'heroCompact' ? <DescriptionRecetteCard>{recette.description}</DescriptionRecetteCard> : null}
