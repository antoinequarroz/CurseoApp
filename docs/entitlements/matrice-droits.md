...
**COUR-39 — la colonne `profils.abonnement` était elle-même modifiable par le client** : la garde serveur ajoutée en COUR-35 lit `profils.abonnement` pour décider si l'ajout d'un membre du foyer est autorisé. Or la seule policy sur `profils` (`profil_own ... for all using (auth.uid() = id)`) ne garantit que la **propriété** de la ligne, jamais l'intégrité de ses colonnes : un utilisateur authentifié pouvait s'auto-attribuer n'importe quel palier avec un `PATCH /rest/v1/profils?id=eq.<son_uid> {"abonnement":"famille"}`, puis ajouter des membres — la garde COUR-35 était donc entièrement contournable en deux requêtes. Démontré sur un compte réel non privilégié pendant l'écriture des tests d'intégration (le PATCH renvoyait `204`, le palier passait bien à `famille`, l'ajout de membre passait de `400` à `201`). Corrigé par le trigger `trg_proteger_abonnement_profil` (`20260728000000_profils_garde_abonnement.sql`) : la valeur proposée par le client est **ignorée** (forcée à `gratuit` à l'INSERT, conservée à l'UPDATE) plutôt que rejetée, car l'onboarding envoie légitimement un upsert du profil complet contenant ce champ. Seul `service_role` — donc le webhook RevenueCat, unique source de vérité (ADR-004) — peut encore écrire cette colonne. Non-régression couverte par `scripts/verify-parcours-critiques.sh`.
